...
describe("observability performance budget", () => {
  it(
    "makes representative status and artifact evidence visible within the R0 budgets",
    async () => {
      ...
      expect(await fixture.events.listNormalized(session.id)).toHaveLength(250);
      expect(
        await fixture.artifacts.listSessionArtifacts(fixture.workspaceId, session.id),
      ).toHaveLength(1);
