...
export class SessionReadinessService {
  ...
  assertReady(workspaceId: string, request: SessionStartRequest) {
    const published = this.agentRegistryService.getPublishedAgentForSession(
      workspaceId,
      request.agentId,
    );
    const blockers = this.readinessBlockers(workspaceId, published.agent);
  ...
  readinessBlockers(workspaceId: string, agent: Agent): string[] {
    const health = this.workspaceEnvironmentService.getHealth();
