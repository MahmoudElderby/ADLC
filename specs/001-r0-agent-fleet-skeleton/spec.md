...
- Q: How should the walking skeleton behave when its MCP tool would perform a risky external action? → A: Use the agent's Always allow mode so the action proceeds without an approval flow.
- Q: Which terminal session outcomes must the walking skeleton distinguish? → A: Completed, failed, canceled, and interrupted.
- Q: When the same Markdown file path is reported more than once, what should count as the same artifact? → A: Every artifact report creates a new record, even for the same path.
- Q: Should the walking skeleton enforce different permissions for the platform engineer and operator personas? → A: No. One authenticated workspace user may complete the entire golden path.
- Q: From where should the walking-skeleton MCP connection be reached? → A: The self-hosted workspace connects to the remote HTTP MCP server, including private services reachable through its VPN.

## User Scenarios & Testing *(mandatory)*

### User Story 1 - Prepare a Runnable Agent (Priority: P1)

As an authenticated workspace user acting as a platform engineer, I register one skill and one MCP server, create an agent that uses both capabilities, review its configuration, and publish it so that it is available for controlled execution.
...
1. **Given** a healthy self-hosted workspace with access to a remote HTTP MCP server, **when** the user registers the skill and MCP server, **then** both appear in the Capability Registry with their validation status, the MCP connection origin is identified as the self-hosted workspace, and no credential values are displayed.
...
As the same authenticated workspace user acting as an operator, I start a session with the published agent in the shared self-hosted workspace and receive a tracked Markdown artifact so that the agent's useful output has durable provenance.
...
3. **Given** the agent creates a Markdown file in the shared workspace, **when** the platform receives an artifact report, **then** it creates a new record containing the artifact name, type, file reference, producing agent, producing session, report occurrence, timestamps, and status.
...
5. **Given** the published agent uses Always allow, **when** its MCP tool performs a risky external action, **then** the action proceeds without pausing for approval and its tool activity is retained in the session trace.

---

### User Story 3 - Monitor and Investigate the Run (Priority: P3)

As the same authenticated workspace user acting as an operator, I see the active session in Command Center and inspect its trace and audit history so that I can determine what happened, which configuration was used, and where the output was produced.
...
2. **Given** a session becomes completed, failed, canceled, or interrupted, **when** Command Center refreshes, **then** the session is removed from the active fleet canvas and remains accessible from session history or a relevant status summary.
...
- An MCP server is saved but cannot be reached from the self-hosted workspace because its VPN route is unavailable, or its referenced credential is missing or unhealthy.
...
- The same artifact reference is reported more than once during or across sessions; each report remains a distinct provenance record.
...
- **FR-003**: The system MUST allow a user to register a remote HTTP MCP server with its connection details, allowed tools, required status, self-hosted-workspace connection origin, and credential reference without displaying or retaining a browser-readable credential value.
...
- **FR-005**: The system MUST allow a user to create a draft agent with a name, purpose, instructions, attached skill, attached MCP server, and risky-action approval mode; the walking-skeleton acceptance agent MUST use Always allow.
...
- **FR-012**: Before starting a session, the system MUST verify that the workspace and required agent capabilities are available, including MCP reachability from the self-hosted workspace, and MUST explain any condition that blocks execution.
...
- **FR-016**: After a session becomes completed, failed, canceled, or interrupted, Command Center MUST remove it from the active fleet canvas while preserving navigation to its historical record through the appropriate session or status view.
- **FR-017**: Whenever a session reports a valid Markdown file in the shared workspace, the system MUST create a new artifact record even when an earlier report references the same file path.
- **FR-018**: Each artifact record MUST identify the artifact name, Markdown type, workspace file reference, status, producing agent, producing session, report occurrence, and relevant timestamps.
...
- **FR-023**: The system MUST preserve partial trace and audit evidence when a session fails, is canceled, is interrupted, or loses live status delivery.
...
- **FR-025**: Retrying display or status delivery MUST NOT create duplicate session, event, or audit records; each distinct artifact report MUST create exactly one artifact record.
- **FR-026**: When the acceptance agent's MCP tool performs a risky external action, the system MUST proceed without creating an approval request and MUST retain the tool activity in the session trace.
- **FR-027**: The system MUST represent completed, failed, canceled, and interrupted as distinct terminal session outcomes.
- **FR-028**: The system MUST allow one authenticated workspace user to register capabilities, create and publish the agent, start the session, and inspect status, trace, artifacts, and audit history without enforcing separate platform-engineer and operator roles in this feature.
- **FR-029**: MCP tool traffic for this feature MUST originate from the self-hosted workspace so the agent can use MCP servers reachable only through the workspace's private network or VPN.
...
- **MCP Server**: A reusable remote HTTP tool connection registered once and attached to agents by reference, including allowed tools, self-hosted-workspace connection origin, reachability status, and an opaque credential reference.
...
- **Session**: One execution of a published agent for an initial task, with lifecycle state, effective redacted configuration snapshot, trace, artifact references, and one of four terminal outcomes: completed, failed, canceled, or interrupted.
...
- **Artifact**: Metadata and provenance for one report of a Markdown file produced in the shared workspace. Its identity is the report occurrence rather than the file path, so separate reports of the same path remain distinct records.
...
- **SC-005**: For 100% of tested completed, failed, canceled, and interrupted sessions, a reviewer can determine the initiating actor, agent configuration, significant execution events, artifact outcome, and final session state from retained records.
...
- **SC-008**: Repeated delivery of the same status event creates zero duplicate session, event, or audit records, while each distinct artifact report creates exactly one artifact record in acceptance testing.

## Assumptions

- Workspace authentication and one authorized workspace-user identity already exist. The platform-engineer and operator labels describe user jobs rather than separately enforced roles; identity administration and role-based access control are outside this feature.
...
- The walking skeleton registers one skill and one remote HTTP MCP server that is reached from the self-hosted workspace; broader capability lifecycle management and additional connection types or origins are deferred.
- The self-hosted workspace has the required private-network or VPN route to the acceptance MCP server. Loss of that route makes MCP validation or session readiness fail visibly.
...
- Workflow composition, Platform Chat, multi-agent delegation, approval-request handling, multiple environments, OpenAI-hosted environments, advanced analytics, and complete registry administration are outside this feature.
