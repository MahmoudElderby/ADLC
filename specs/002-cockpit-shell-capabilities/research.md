...
**Alternatives considered**: Copying the Figma Make prototype wholesale was rejected by the UI guidelines. A CSS-in-JS second system would fight Tailwind. Leaving primitives in `apps/web` would repeat spec 001's stub `@adlc/ui`.
...
- `WorkspaceUserGuard` reads the cookie, loads the hashed session, rejects missing/expired/revoked sessions with 401 and an empty body that discloses no workspace data, and derives `workspaceId` and `actorId` from the session row. Request headers that name a workspace or actor are ignored.
...
3. **Typed checks**: Registering or revalidating an MCP server inserts `environment_checks` with `check_type = mcp_reachability`, `status = pending`, an expiry (10 seconds to match SC-008), the approved MCP id, URL, allowed tools, and — in memory on the TLS response to the connector only — the decrypted credential. The connector polls `GET /api/v1/connectors/{id}/checks/next`. It performs a real HTTP MCP handshake from the environment, redacts errors locally, and posts `POST .../checks/{id}/result`. Unexpected check ids, expired checks, and checks the platform did not issue are rejected. The connector does not accept generic commands in this slice.
